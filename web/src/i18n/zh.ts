...
export const zh = {
  ...
  setup_apikey_purpose: '这把 key 等同于你的登录身份，可调用 Scout 的全部 HTTP API。最省事的用法：把它和本实例地址一起发给你的 AI 助手，让助手替你把 TMDB、大模型、字幕源等凭证逐项填好、添加媒体目录，之后也能随时帮你查巡检进展、盯容器健康。',
  setup_apikey_notice: '仅此一次完整显示。此后设置页只显示末 4 位，你可以在那里随时复制或重新生成。',
  setup_apikey_copy: '复制',
  setup_apikey_copied: '已复制',
  setup_apikey_copy_failed: '复制失败——请直接选中上方文本手动复制',
  ...
  settings_translate_after_label: '几次找不到后移交翻译',
  settings_translate_after_1: '1 次',
  settings_translate_after_3: '3 次',
  settings_translate_after_7: '7 次（默认）',
  settings_translate_after_14: '14 次',
  settings_translate_after_30: '30 次',
  settings_translate_after_note: '每天巡检各尝试一次，次数约等于天数。攒满这么多次真实的\u201c确实没找到\u201d后，交给 AI 翻译工作流。',
  ...
  settings_translate_dedicated_note: '自动翻译必须配置专用模型，不会使用默认 LLM。',
  settings_translate_all_fields_required: '三个字段都必须填写',
  settings_translate_save_error_prefix: '无法保存：',
  ...
  unidentified_note: '这些目录试过了还是认不出来',
  /** P6（2026-09-23）：**按"我们到底试到什么程度"分两档说**，不再用一句通用话。
   *  原来那句「目录名清晰可辨最有帮助；改名后下轮巡检会重试」对下列处境是**误导**：
   *   · exhausted —— agent 已搜过全名与去噪变体、TMDB 上确实没有：改名白费力气，
   *     该走的是"指定作品"（目录名本身就是那个入口）；
   *   · transient —— 这一轮没成（超时/抖动），**没有**任何"搜遍了"的证据：用户什么都不用做。 */
  unidentified_reason_exhausted: '已经搜遍了目录名及其去噪变体，TMDB 上没有对应作品——改名帮不上忙；知道它是哪部作品的话，点上面的目录名指定一下即可',
  unidentified_reason_transient: '这一轮没跑成（超时或抖动），下一轮会自动重试，暂时不用管',
  unidentified_reason_mixed: '上面既有"搜遍了也没找到"的，也有"这轮没跑成"的——前者请点目录名指定作品，后者会自动重试',
