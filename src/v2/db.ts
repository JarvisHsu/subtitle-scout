...
CREATE INDEX IF NOT EXISTS notifications_found_at ON notifications(found_at DESC);
  ...
  // `media_roots=0` 的库上**每一张表都是 0 行**，行数只说明"到目前为止没人往里写过东西"，
  // 而那既可能是"这条路已经死了"，也可能是"这条路还没被走过"——两者在行数上完全不可分辨。
  //
  // ⚠️ 2026-08-14 再更正：此处**原本举的例子是错的**。原文写「本仓的活表 `item_files`
  // 同样 0 行，而它有真实写入方」——T1 取证实测推翻：`addItemFile` 的唯一调用者是
  // `ingest.ts`，而那个文件**已被物理删除**，全仓剩下的 `addItemFile` 提及只在注释里。
  // 也就是说 `item_files` 不是"活表恰好 0 行"，它是**真的没有写入方了**。
  // 拿一张死表去论证"0 行不代表死"，论据本身是空的——结论对，例子错。
  //
  // 换成一个经实测站得住的例子：`series` 表同样生产 0 行，而它的写入方
  // `libraryRepo.upsertSeries` 是**活代码**——2026-08-13 注入 throw 实测红 514 条 / 14 文件
  // （三个活 HTTP 端点 + realign 圣文件 + removeRoot 的验证能力都塌了）。
  // 本仓因此拒绝过一条审计建议：`upsertSeries` 三件套被判「死代码、生产 0 行」，
