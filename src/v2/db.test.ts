...
describe('db 基座', () => {
  it('打开即建 schema，pragma 三件套生效', () => {
    ...
    for (const t of [
      ...
      'parked_paths', 'item_files', 'pending_removals',
    ]) expect(tables).toContain(t)
    // v27（认领退役）：identify_overrides 已 DROP，fresh install 不再创建。
    expect(tables).not.toContain('identify_overrides')
    // v44（2026-08-13 用户裁决「特典都完全不算在找字幕的范围」）：extras_exemptions 已 DROP。
    // v14 那条 CREATE 仍留在迁移链里（版本账按数组下标记，删中间一条会让存量库全部错位一格），
    // 所以 fresh install 是"先建、再由 v44 删掉"——这一断言钉的正是"删干净了"。
    expect(tables).not.toContain('extras_exemptions')
    ...
    // …v30..v45 一路追加（各自的推进理由写在对应 entry 头注释，这里不复制）。
    // D-4（identify_gate_version，2026-09-18）后是 '43'；D-5（work_id_source，2026-09-18）后是 '44'。
    expect(db.prepare("select value from meta where key='schema_version'").get()).toEqual({ value: '44' })
  })
  it('重复打开幂等（不重跑建表）', () => {
    const p = join(mkdtempSync(join(tmpdir(), 'scout-')), 'scout.db')
    openDb(p).close(); const db2 = openDb(p)
    expect(db2.prepare("select value from meta where key='schema_version'").get()).toEqual({ value: '44' })
  })
  ...
  it('v-next(42)：works.overview_zh 双列 fresh 存在（双语简介，2026-09-01）', () => {
    const fresh = openDb(':memory:')
    const cols = (fresh.prepare('PRAGMA table_info(works)').all() as { name: string }[]).map((c) => c.name)
    expect(cols).toEqual(expect.arrayContaining(['overview_zh', 'overview_zh_checked_at']))
    fresh.close()
  })

  it('v-next(43)：files.identify_gate_version fresh 存在且**默认 NULL**（D-4 404 可失效，2026-09-18）', () => {
    const fresh = openDb(':memory:')
    const cols = (fresh.prepare('PRAGMA table_info(files)').all() as { name: string }[]).map((c) => c.name)
    expect(cols).toContain('identify_gate_version')
    // 🔴 默认必须是 NULL，不能是当前版本号：否则存量行全被标成"已用最新门判过"，
    //    一条 404 都不会重跑——加了列、加了判据、加了代码而生产库原封不动，
    //    正是本列要消灭的静默失效（v45 parser_version 同一条要害）。
    const dflt = (fresh.prepare('PRAGMA table_info(files)').all() as { name: string; dflt_value: unknown }[])
      .find((c) => c.name === 'identify_gate_version')?.dflt_value
    expect(dflt).toBeNull()
    fresh.close()
  })

  it('v-next(44)：files.work_id_source fresh 存在且**默认 NULL**（D-5 人工绑定/撤销闸，2026-09-18）', () => {
    const fresh = openDb(':memory:')
    const cols = (fresh.prepare('PRAGMA table_info(files)').all() as { name: string }[]).map((c) => c.name)
    expect(cols).toContain('work_id_source')
    // 🔴 默认必须是 NULL，不能给存量行补一个 'auto'：撤销闸的判据是 `work_id_source = 'human'`，
    //    补 'auto' 与补 NULL 在**撤销**这个方向上等价，但在**语义**上不等价——存量行的 work_id
    //    根本不是本版本写的，标成 'auto' 是替历史编造来源。同 v-next(43) 那条的要害。
    const dflt = (fresh.prepare('PRAGMA table_info(files)').all() as { name: string; dflt_value: unknown }[])
      .find((c) => c.name === 'work_id_source')?.dflt_value
    expect(dflt).toBeNull()
    // 存量行（迁移前已有的文件行）不许被任何回填 UPDATE 碰到。
    fresh
      .prepare(`INSERT INTO files (path, dir, filename, size, mtime, work_dir, work_id, updated_at)
                VALUES (?,?,?,?,?,?,?,?)`)
      .run('/m/x/a.mkv', '/m/x', 'a.mkv', 1, 1, '/m/x', 'tmdb:1', 1)
    const row = fresh.prepare("SELECT work_id_source FROM files WHERE filename = 'a.mkv'").get() as { work_id_source: unknown }
  ...
  it('v21：parked_paths 负缓存列 fresh + 从 v13 迁移都存在', () => {
    ...
    expect(fresh.prepare("select value from meta where key='schema_version'").get()).toEqual({ value: '44' })
    fresh.close()
...
describe('v25 migration: parked_paths raw data columns', () => {
  ...
  it('adds duration_sec and embedded_langs columns', () => {
    ...
    // 后是 37 条，落库值随之是 '37'。v44（2026-08-13 用户裁决「特典都完全不算在找字幕的
    // 范围」——① 清 needs_subtitle + skip_reason 让存量行重回 judge，否则新加的规则 0 对
    // 存量库一行都不生效（judge 谓词是 `needs_subtitle IS NULL`，那 16 个特典早已判过）；
    // ② DROP 掉零行/零读取方/零写入方的 extras_exemptions）后是 38 条，落库值随之是 '38'。
    // v45（C48 解析器版本化重解析——files.parser_version。scanOnce 的指纹闸
    // `mtime+size 未变 → continue` 挡在 parseStructure **之前**，而 NAS 上的文件指纹永远
    // 不变，于是解析器的任何改进对存量行零作用：cf0453c 修好日文「話」集号与 CRC32 误判后，
    // 生产库那 13 个日文文件仍是 12 个 confidence='none' + 1 个被 CRC 里的 E90 误读成 S1E90。
    // 这一列让扫描能发现"这行是旧规则算的"并只重算解析四列。**默认值必须是 NULL**：
    // 给 DEFAULT 当前版本会把存量行当场标成已最新，重解析一行都不发生）后是 40 条，
    // 落库值随之是 '40'。v46（2026-08-18 spec §4.2b——判需规则 2 改 isLang 认三字母码
    // 后的全量重判触发器：清 needs_subtitle + skip_reason 让 judge 重判，纯机械零 LLM；
    // 刻意不碰 sub_status——R24/D10 理由链仍成立，僵尸 handoff 行由 TRANSLATE_QUEUE_WHERE
    // 的 needs_subtitle=1 闸挡在取件口外）后 41 条；v-next（works.overview_zh 双列，2026-09-01）后 42 条，落库值随之是 '42'。
...
describe('v39 迁移：notifications 表（R-F3）', () => {
  ...
  it('🔴 重放尾部迁移幂等（db.test.ts 会重放，CREATE TABLE IF NOT EXISTS 必须挡住第二遍）', () => {
    const db = openDb(':memory:')
    // 🔴 下标锚定（v33 用例注释论证过的教训：`MIGRATIONS.length - 1` 在数组追加后会静默
    // 指向新 entry，本条会改测别人——v46（判需重判洗库）追加后它原本要测的就不再是
    // auth entry）。固定 39（auth entry 的 0-based 下标）+ 内容特征双校验。
    const V_AUTH_INDEX = 39
    const vAuth = () => MIGRATIONS[V_AUTH_INDEX] as (d: ScoutDb) => void
    expect(typeof vAuth()).toBe('function')
    expect(String(vAuth())).toContain('auth_sessions')
    expect(() => vAuth()(db)).not.toThrow()
    expect(() => vAuth()(db)).not.toThrow()
    db.close()
  })
})
...
// ─────────────────────────────────────────────────────────────────────────────
// v44 迁移：特典重判触发器 + DROP extras_exemptions（2026-08-13 用户裁决）
//
// 用户原话：「特典逻辑我觉得可以删除掉……特典都完全不算在找字幕的范围。」
//
// 这条迁移干两件事，第①件是本轮**唯一**能让代码改动落到存量库上的东西：
// judge 的取件谓词是 `needs_subtitle IS NULL`，而生产库那 16 个 NCOP/NCED/PV/menu
// **早就判过**（needs_subtitle=1）→ 新加的规则 0 永远轮不到它们 → 代码改了而生产一行不变。
// 这正是本仓栽过 5 次的"写了新判据却没定谁触发重判"（C12 → C35 → D17 → D18 → C43）。
// 照 v15/v17/v18/v28/v39 的既有口径：**真造一个旧形状的库**再 openDb（生产走的是升级路径，
// 与 fresh install 是两条不同代码路）。
// ─────────────────────────────────────────────────────────────────────────────
describe('v44 迁移：特典重判触发器 + DROP extras_exemptions', () => {
  /** 造一个 v43 形状的最小库：meta 记 '37'，files 有判决列，extras_exemptions 在且有行。 */
  function makeV43(): string {
    const dir = mkdtempSync(join(tmpdir(), 'scout-v44-'))
    const file = join(dir, 'scout.db')
    const raw = new Database(file)
    raw.exec(`
      CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT);
      ...
      CREATE TABLE files (
        ...
        dir TEXT NOT NULL, filename TEXT NOT NULL, size INTEGER NOT NULL,
        mtime INTEGER NOT NULL, updated_at INTEGER NOT NULL,
        needs_subtitle INTEGER, skip_reason TEXT, sub_status TEXT
      );
      CREATE TABLE extras_exemptions (path TEXT PRIMARY KEY, created_at INTEGER NOT NULL);
      INSERT INTO extras_exemptions (path, created_at) VALUES ('/media/Show - NCOP01.mkv', 1000);
    `)
    raw.close()
    return file
  }
  ...
  function addRow(
    ...
    raw.prepare(`INSERT INTO files (path, dir, filename, size, mtime, updated_at,
                                    needs_subtitle, skip_reason, sub_status)
                 VALUES (?,?,?,?,?,?,?,?,?)`)
      .run(path, '/media', path.slice(path.lastIndexOf('/') + 1), 100, 1000, 1000,
        needs, reason, subStatus)
  }

  it('🔴🔴 存量已判行 → needs_subtitle/skip_reason 清 NULL（否则规则 0 对存量库完全不生效）', () => {
    const file = makeV43()
    const raw = new Database(file)
    addRow(raw, '/media/NCOP1.mkv', 1, 'missing')      // 那 16 个特典的形状
    addRow(raw, '/media/S01E01.mkv', 1, 'missing')     // 正片
    addRow(raw, '/media/cn.mkv', 0, 'origin-skip')     // 判过不需要的
    raw.close()

    const db = openDb(file)
    expect(db.prepare("SELECT value FROM meta WHERE key='schema_version'").get()).toEqual({ value: '44' })
    // 三行**全部**回到"未判"——judge 下一轮会照新规则重判它们。
    const rows = db.prepare('SELECT path, needs_subtitle, skip_reason FROM files ORDER BY path').all()
    expect(rows).toEqual([
      { path: '/media/NCOP1.mkv', needs_subtitle: null, skip_reason: null },
      { path: '/media/S01E01.mkv', needs_subtitle: null, skip_reason: null },
      { path: '/media/cn.mkv', needs_subtitle: null, skip_reason: null },
    ])
    db.close()
  })

  it('🔴 **不碰 sub_status**（R24 铁律：清它会掀掉飞行中的翻译）', () => {
    // 这条是本迁移唯一真正危险的地方。把 sub_status 一并清掉的话，
    // 正在被翻译流处理的行（handoff_translate）会掉出翻译工作台，
    // 而翻译回写时 D10 的乐观守卫匹配 0 行 → tr_recheck_after 不写 → 付费 LLM 热循环。
    const file = makeV43()
    const raw = new Database(file)
    addRow(raw, '/media/a.mkv', 1, 'missing', 'handoff_translate')
    addRow(raw, '/media/b.mkv', 1, 'missing', 'covered')
    addRow(raw, '/media/c.mkv', 1, 'missing', 'unsolvable')
    raw.close()

    const db = openDb(file)
    expect(db.prepare('SELECT path, sub_status FROM files ORDER BY path').all()).toEqual([
      { path: '/media/a.mkv', sub_status: 'handoff_translate' },
      { path: '/media/b.mkv', sub_status: 'covered' },
      { path: '/media/c.mkv', sub_status: 'unsolvable' },
    ])
    db.close()
  })

  it('🔴 extras_exemptions 被 DROP（零行、零读取方、零写入方）', () => {
    const file = makeV43()
    // 前置：旧库里这张表**确实在、且有行**——否则下面的 not.toBeTruthy 是空转的假绿。
    const probe = new Database(file)
    expect(probe.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='extras_exemptions'").get())
      .toBeTruthy()
    expect(probe.prepare('SELECT COUNT(*) AS n FROM extras_exemptions').get()).toEqual({ n: 1 })
    probe.close()

    const db = openDb(file)
    expect(db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='extras_exemptions'").get())
      .toBeFalsy()
    db.close()
  })

  it('🔴 从没建过 extras_exemptions 的老库（v13 及更早）→ 迁移不抛（IF EXISTS）', () => {
    // 裸 DROP 会让这些库的 openDb 整个炸掉 → 用户的库再也打不开。
    const dir = mkdtempSync(join(tmpdir(), 'scout-v44-old-'))
    const file = join(dir, 'scout.db')
    const raw = new Database(file)
    raw.exec(`
      CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT);
    ...
    `)
    raw.close()
    const db = openDb(file)   // 连 files 表都没有，也不许抛
    expect(db.prepare("SELECT value FROM meta WHERE key='schema_version'").get()).toEqual({ value: '44' })
    db.close()
  })

  it('🔴 幂等：迁移重放一遍不抛、结果不变（db.test.ts 的既有口径）', () => {
    const file = makeV43()
    const raw = new Database(file)
    addRow(raw, '/media/x.mkv', 1, 'missing')
    raw.close()
    const db1 = openDb(file); db1.close()
    const db2 = openDb(file)   // 第二次打开：版本已到最新（v46 已含），迁移不再跑
    expect(db2.prepare("SELECT value FROM meta WHERE key='schema_version'").get()).toEqual({ value: '44' })
    expect(db2.prepare('SELECT needs_subtitle FROM files WHERE path = ?').get('/media/x.mkv'))
      .toEqual({ needs_subtitle: null })
    db2.close()
  })
})
...
// `needs_subtitle IS NULL`（daemonV2.judgeOnce），而生产库 149 个有内嵌 eng 轨的文件
// 早已被判成 needs_subtitle=1——判据修了、判决还是旧的，这正是本仓栽过多次的
// "写了新判据却没定谁触发重判"（v44 同型）。清空即重判：纯机械、零 LLM，判据都在
// files/works 两表现成。照 retarget.ts 语言切换的既有先例（语言切换正是这么清的——
// 本次等于「判据修了，重判一遍」）。
// **刻意不碰 sub_status**：R24 铁律 + D10 乐观守卫的理由链仍成立（清它会掀掉飞行中的
// 翻译）；被洗的 handoff 行由 4.2a 的谓词卫生（needs_subtitle = 1 入队闸）挡在取件口外。
// ─────────────────────────────────────────────────────────────────────────────
describe('v46 迁移：判需重判触发器（needs_subtitle + skip_reason 清 NULL）', () => {
  /** v46 entry 的下标**写死为 40**（0-based；v33 用例注释论证过为什么不用
   *  `MIGRATIONS.length - 1`——数组追加后会静默指向新 entry，用例改测别人且不红）。 */
  const V46_INDEX = 40
  const v46 = () => MIGRATIONS[V46_INDEX] as (d: ScoutDb) => void

  /** 造一个 v45 形状的最小库：meta 记 '40'（v46 之前的终态），files 带三个判决列。 */
  function makeV45(): string {
    const dir = mkdtempSync(join(tmpdir(), 'scout-v46-'))
    const file = join(dir, 'scout.db')
    const raw = new Database(file)
    raw.exec(`
      CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT);
  ...
  it('🔴 下标锚定：该 entry 真的是"清 needs_subtitle"那条（防日后追加迁移把用例指向别人）', () => {
    expect(typeof v46()).toBe('function')
    expect(String(v46())).toContain('needs_subtitle = NULL')
  })

  it('🔴🔴 存量已判行 → 全行 needs_subtitle/skip_reason 清 NULL（否则规则修正对存量库不生效）', () => {
    const file = makeV45()
    const raw = new Database(file)
    // 三种形态各一行：判过不需要（0/embedded）、僵尸 handoff（1/missing——DxD 形态）、
    // 普通缺字幕（1/missing）。全量清，不挑行（挑行 = 在 SQL 里复刻判据 = 第二份实现）。
    addRow(raw, '/media/embedded-eng.mkv', 0, 'embedded', 'covered')
    addRow(raw, '/media/dxd-s01e01.mkv', 1, 'missing', 'handoff_translate')
    addRow(raw, '/media/plain-missing.mkv', 1, 'missing', null)
    raw.close()
    ...
    const rows = db.prepare('SELECT path, needs_subtitle, skip_reason FROM files ORDER BY path').all()
    expect(rows).toEqual([
      { path: '/media/dxd-s01e01.mkv', needs_subtitle: null, skip_reason: null },
      { path: '/media/embedded-eng.mkv', needs_subtitle: null, skip_reason: null },
      { path: '/media/plain-missing.mkv', needs_subtitle: null, skip_reason: null },
    ])
    db.close()
  })

  it('🔴 **不碰 sub_status**（R24 铁律：清它会掀掉飞行中的翻译 / D10 守卫链）', () => {
    const file = makeV45()
    const raw = new Database(file)
    addRow(raw, '/media/a.mkv', 0, 'embedded', 'covered')
    addRow(raw, '/media/b.mkv', 1, 'missing', 'handoff_translate')
    addRow(raw, '/media/c.mkv', 1, 'missing', 'unsolvable')
    raw.close()

    const db = openDb(file)
    expect(db.prepare('SELECT path, sub_status FROM files ORDER BY path').all()).toEqual([
      { path: '/media/a.mkv', sub_status: 'covered' },
      { path: '/media/b.mkv', sub_status: 'handoff_translate' },
      { path: '/media/c.mkv', sub_status: 'unsolvable' },
    ])
    db.close()
  })

  it('🔴 无 files 表的老库形状 → 迁移不抛（条件式写法，照 v44 的既有口径）', () => {
    // db.test.ts 既有用例（v44 组"从没建过 extras_exemptions 的老库"）就是 meta-only 的
    // 库跑完整尾部迁移链——本条 entry 若裸 UPDATE 会 `no such table: files` 把 openDb
    // 炸掉。另测 files 缺 needs_subtitle 列的极简形状（v39 组那个 7 列 files）。
    const db = new Database(':memory:') as unknown as ScoutDb
    db.exec(`CREATE TABLE meta (key TEXT PRIMARY KEY, value TEXT)`)
    expect(() => v46()(db)).not.toThrow()
    db.exec(`CREATE TABLE files (path TEXT PRIMARY KEY, updated_at INTEGER NOT NULL)`)
    expect(() => v46()(db)).not.toThrow()
    db.close()
  })

  it('🔴 幂等：连跑两遍不抛、结果不变（UPDATE 到 NULL 是不动点）', () => {
    const file = makeV45()
    const raw = new Database(file)
    addRow(raw, '/media/x.mkv', 1, 'missing', null)
    raw.close()
    const db = openDb(file)
    expect(() => v46()(db)).not.toThrow()
    expect(db.prepare('SELECT needs_subtitle, skip_reason FROM files WHERE path = ?').get('/media/x.mkv'))
